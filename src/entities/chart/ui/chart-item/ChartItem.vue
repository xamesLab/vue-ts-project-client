...
export default {
    ...
    watch: {
        'wsData.close': {
            handler(close) {
                state.value = { count: Number(close) }
                const myEvent = new CustomEvent("ev", { detail: { username: "davidwalsh" } })
                window.dispatchEvent(myEvent)
            },
            deep: true
        },
        pairData() {
            if(!this.isInit) this.init(this.pairData.BNBUSDT)
            this.isInit = true
        }
    },
    computed: {
    ...
    methods: {
        init(data) {
            ...
            const setNewValue = (newValue) => {
                ...
                if (lastDataObject) {
                    var previousDate = lastDataObject.openTime;
    ...
    mounted() {
        useCandleModel().fetchCandles({symbol: 'BNBUSDT', interval: '15m', limit: 100})
    }
}
</script>
<style>
